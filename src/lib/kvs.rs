...
use crate::log::{create_log_file, LogCommand, LogReader, LogWriter};

pub type KVSResult<T> = Result<T, Error>;
...
type LenInLog = usize;
type KvStoreValue = (LogName, PositionInLog, LenInLog);
...
impl KvStore {
    ...
    fn get_log_files_names_by_path(dir: ReadDir) -> KVSResult<Vec<String>> {
        let mut files: Vec<String> = dir
            .flat_map(|res| -> KVSResult<_> { Ok(res?.path()) })
...
mod tests {
    ...
    where
        T: FnOnce(&Path) -> KVSResult<()> + panic::UnwindSafe,
    {
